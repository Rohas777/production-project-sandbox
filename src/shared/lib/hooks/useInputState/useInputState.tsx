...
export interface UseInputStateReturnValue<T> {
    value: T,
    onChange: (value: null | undefined | T | React.ChangeEvent<any>) => void,
}

export function useInputState<T>(initialState: T): UseInputStateReturnValue<T> {
    const [value, setValue] = useState<T>(initialState);
    return { value, onChange: getInputOnChange<T>(setValue as any) };
}
